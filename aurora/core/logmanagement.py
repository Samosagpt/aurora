...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(config.LOGS_DIR / "aurora.log"), logging.StreamHandler()],
)
...
"""
        ...
        try:
            ...
            if self.conversation_log_path.exists():
                ...
                backup_path.write_text(
                    self.conversation_log_path.read_text(encoding="utf-8"), encoding="utf-8"
                )
    ...
    def export_logs(self, export_path: Optional[Path] = None) -> Path:
        ...
        if export_path is None:
            export_path = (
                config.LOGS_DIR / f"export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
        ...
        try:
            ...
            if self.md_log_path.exists():
                (export_path / "assistant_log.md").write_text(
                    self.md_log_path.read_text(encoding="utf-8"), encoding="utf-8"
                )
