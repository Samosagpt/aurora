...
from aurora.core import Generation as gen
...
try:
    from aurora.core.config_prod import config
except ImportError:
    from aurora.core.config import config

try:
    from aurora.handlers.error_handler import (
        error_handler,
        performance_monitor,
        production_error_handler,
    )
    from aurora.security.health_check import health_check
    from aurora.security.security import audit_logger, rate_limited, security_manager
...
try:
    from aurora.audio.offline_sr_whisper import speech_recognition
...
try:
    from aurora.audio.offline_text2speech import speak_text
...
from aurora.utils.user_preferences import get_preferences_manager
from streamlit_markdown_select import create_option, markdown_select
from streamlit_navbar import create_nav_item, navbar

# Import attachment handler
try:
    from aurora.handlers.attachment_handler import attachment_handler
...
try:
    from aurora.handlers.prompthandler import prompt_handler
...
try:
    from aurora.agents.agentic_handler import (
        agentic_handler,
        handle_agentic_request,
        is_desktop_control_request,
    )
...
try:
    from aurora.agents.vision_agent import execute_autonomous_task, vision_agent
...
try:
    from aurora.handlers.rag_handler import RAGHandler, get_rag_handler
...
try:
    from aurora.core.aurora_system import AuroraSystem, get_aurora_system
...
try:
    from aurora.models.video_model_manager import video_model_manager
...
try:
    from aurora.utils.hardware_optimizer import (
        get_chat_settings,
        get_hardware_info,
...
def configure_streamlit():
    ...
    try:
        ...
        logo_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "_assets", "aurora_logo.jpg"
        )
...
def chat_page():
    ...
    with col_title2:
        if AGENTIC_HANDLER_AVAILABLE:
            agentic_mode = st.toggle(
                ...
                help="Enable basic desktop control",
    ...
    with col_title3:
        if VISION_AGENT_AVAILABLE:
            vision_mode = st.toggle(
                "�️ Vision Agent",
                value=st.session_state.get("vision_mode", False),
                help="Enable autonomous vision-guided control",
                key="vision_mode_toggle",
            )
    ...
    try:
        ...
        for i, model_name in enumerate(available_models):
            is_recommended = (
                model_name in recommended_models if HARDWARE_OPTIMIZER_AVAILABLE else False
            )
            is_default = model_name == default_model

            # Create option with status indicator and buttons
            buttons = [{"text": "🗑️ Delete", "variant": "danger", "action": "delete"}]
        ...
        if (
            ...
            and component_value.get("type") == "button_click"
        ):
            ...
            if button_data:
                ...
                if action == "delete" and option_value:
                    ...
                    if not st.session_state[delete_key]:
                        ...
                        with col1:
                            if st.button(
                                "🗑️ Yes, Delete", type="primary", key=f"btn_confirm_{option_value}"
                            ):
                        ...
                        with st.spinner(f"Deleting {option_value}..."):
                            try:
                                ...
                                keys_to_remove = [
                                    ...
                                    if key.startswith("chat_model_select")
                                ]
        ...
        if isinstance(current_last_used, dict):
            current_last_used = current_last_used.get("value") or current_last_used.get(
                "optionValue"
            )
        ...
        if not model:
            ...
            st.info(
                "Or use the 'Install New Model' option above to install a model from the Ollama registry."
            )
    ...
    with col3:
        ...
        enable_streaming = st.checkbox(
            "Stream Response", value=preferred_streaming, key="enable_streaming"
        )
        ...
        with st.expander("🔊 TTS Settings"):
            try:
                ...
                if available_engines:
                    ...
                    preferred_engine = chat_prefs.get(
                        "preferred_tts_engine", available_engines[0] if available_engines else None
                    )
                    engine_index = 0
                    ...
                    if _preferences_manager and selected_engine != chat_prefs.get(
                        "preferred_tts_engine"
                    ):
                        _preferences_manager.set_preference(
                            "chat", "preferred_tts_engine", selected_engine
                        )
                        _preferences_manager.save_preferences()
                    ...
                    if selected_engine == "bark":
                        ...
                        if _preferences_manager and selected_voice != chat_prefs.get(
                            "preferred_bark_voice"
                        ):
                            _preferences_manager.set_preference(
                                "chat", "preferred_bark_voice", selected_voice
                            )
                            _preferences_manager.save_preferences()
    ...
    with col2:
        ...
        if ATTACHMENT_HANDLER_AVAILABLE:
            ...
            is_multimodal = any(
                mm in model.lower() for mm in [m.split(":")[0] for m in multimodal_models]
            )
            ...
            if not is_multimodal:
                if st.button(
                    ...
                    type="primary",
                    help="Current model doesn't support images",
                ):
    ...
    with st.expander("🎤 Voice Input"):
        ...
        with col1:
            mic_sensitivity = st.slider(
                "Microphone Sensitivity", min_value=0.01, max_value=2.0, value=0.5, step=0.01
            )
        with col2:
    ...
    if ATTACHMENT_HANDLER_AVAILABLE:
        with st.expander("📎 Attachments (Images & PDFs)"):
            st.info(
                "💡 Attach images or PDFs to ask questions about them. Requires a vision model like llava."
            )
                    ...
                    if attachment_data:
                        ...
                        with col1:
                            if attachment_data["type"] == "image":
                                st.image(
                                    f"data:image/png;base64,{attachment_data['data']}",
                                    width=100,
                                    caption=attachment_data["file_name"],
                                )
                        ...
                        with col2:
                            if attachment_data["type"] == "image":
                                st.success(
                                    f"✅ {attachment_data['file_name']} ({attachment_data['size'][0]}x{attachment_data['size'][1]})"
                                )
                            elif attachment_data["type"] == "pdf":
            ...
            if st.session_state["attachments"]:
                ...
                for i, att in enumerate(st.session_state["attachments"]):
                    with cols[i % 4]:
                        if att["type"] == "image":
                            st.image(
                                f"data:image/png;base64,{att['data']}",
                                width=100,
    ...
    if prompt:
        ...
        print(
            f"🔍 DEBUG: Agentic mode in session_state: {st.session_state.get('agentic_mode', 'NOT SET')}"
        )
        print(
            f"🔍 DEBUG: Vision mode in session_state: {st.session_state.get('vision_mode', 'NOT SET')}"
        )
            ...
            if stats["total_documents"] > 0:
                ...
                if search_results:
                    ...
                    if search_results[0]["score"] >= 0.3:
                        logger.info(
                            f"✅ RAG: Found highly relevant content (score: {search_results[0]['score']:.2f})"
                        )

                        # Query RAG system for complete answer
                        try:
                            rag_result = rag_handler.query(
                                prompt, context_window=min(len(search_results), 3)
                            )
                            ...
                            if isinstance(rag_result, dict):
                                ...
                                if sources:
                                    ...
                                    logger.info(
                                        f"✅ RAG: Generated answer from {len(sources)} sources"
                                    )
                            else:
                            ...
                            logger.warning(
                                f"⚠️ RAG query failed, falling back to context enhancement: {e}"
                            )
...
Answer the user's question now:"""
                        ...
                        logger.info(
                            f"✅ RAG: Enhanced prompt with {len(search_results)} context chunks"
                        )
                else:
    ...
    with st.chat_message("assistant"):
        if prompt == "test":
            ...
            if st.session_state.get("enable_streaming", True):
                ...
                # Stream the response
                try:
                    for chunk in gen.ollama_manager.chat_with_memory_stream(
                        prompt, model, images=images
                    ):
                        if chunk:  # Only process non-empty chunks
                            ...
                            if "<think>" in full_response and "</think>" in full_response:
                                ...
                                processed_response = gen.ollama_manager._process_think_tags(
                                    full_response
                                )
                                response_placeholder.markdown(
                                    processed_response, unsafe_allow_html=True
                                )
                            else:
                ...
                try:
                    ...
                    if clean_response.strip():
                        audio_path = speak_text(
                            clean_response, voice_preset=voice_preset, return_path=True
                        )
                    else:
...
def process_agentic_input(prompt, model, attachments=None):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            print(
                f"🔍 DEBUG: Calling handle_agentic_request with prompt: '{prompt}', model: '{model}'"
            )
...
def image_generation_page():
    ...
    try:
        from aurora.ui import image_gen
...
def video_generation_page():
    ...
    with st.sidebar:
        ...
        if VIDEO_MODEL_MANAGER_AVAILABLE:
            ...
            not_installed = [
                model for model in video_models.keys() if model not in installed_models
            ]
            if not_installed:
        ...
        if torch and torch.cuda.is_available():
            ...
            if st.button("🔄 Reset All Models", key="reset_models"):
                ...
                keys_to_remove = [
                    key for key in st.session_state.keys() if key.startswith("video_model_")
                ]
                for key in keys_to_remove:
                    del st.session_state[key]
                if torch.cuda.is_available():
    ...
    with col1:
        ...
        if (
            preferred_model
            and isinstance(preferred_model, str)
            and preferred_model in video_models.keys()
        ):
                    ...
                    if not st.session_state[delete_key]:
                        ...
                        with col1_del:
                            if st.button(
                                "🗑️ Yes, Delete",
                                type="primary",
                                key=f"btn_confirm_video_{option_value}",
                            ):
                                st.session_state[delete_key] = True
                                st.rerun()
                        with col2_del:
                        ...
                        with st.spinner(f"Deleting {option_value}..."):
                            ...
                            keys_to_remove = [
                                key
                                for key in st.session_state.keys()
                                if key.startswith("video_model_select")
                            ]
                            for key in keys_to_remove:
                                if key in st.session_state:
                                    del st.session_state[key]
                ...
                if st.button("➕ Add Model", type="primary"):
                    if custom_model_name and custom_model_id:
                        with st.spinner(f"Adding {custom_model_name}..."):
                            success = video_model_manager.add_custom_model(
                                custom_model_name, custom_model_id
                            )
        ...
        if _preferences_manager and selected_model_name != current_last_used:
            _preferences_manager.set_preference(
                "video_generation", "last_used_model", selected_model_name
            )
            _preferences_manager.save_preferences()
    ...
    with st.expander("🔧 Advanced Video Settings (Hardware Optimized)"):
        ...
        with col_adv1:
            ...
            if _preferences_manager and guidance != video_prefs.get("preferred_guidance"):
                _preferences_manager.set_preference(
                    "video_generation", "preferred_guidance", guidance
                )
                _preferences_manager.save_preferences()
        ...
        with col_adv2:
            ...
            seed = None
            if use_seed:
                seed = st.number_input("Seed Value", min_value=0, max_value=2147483647, value=42)
            ...
            preferred_resolution = video_prefs.get(
                "preferred_resolution", [video_resolution[0], video_resolution[1]]
            )
            preferred_height = (
                preferred_resolution[0] if len(preferred_resolution) >= 1 else video_resolution[0]
            )
            preferred_width = (
                preferred_resolution[1] if len(preferred_resolution) >= 2 else video_resolution[1]
            )
            ...
            if _preferences_manager and current_resolution != video_prefs.get(
                "preferred_resolution"
            ):
                _preferences_manager.set_preference(
                    "video_generation", "preferred_resolution", current_resolution
                )
                _preferences_manager.save_preferences()
            ...
            with st.spinner(f"🔄 Loading {selected_model_name} (this may take several minutes)..."):
                try:
                        ...
                        try:
                            ...
                            for approach_name, load_kwargs in load_approaches:
                                try:
                                    ...
                                    st.info(
                                        f"⚠️ {approach_name} failed: {str(approach_error)[:100]}..."
                                    )
                                    continue

                            if pipeline is None:
                                raise Exception(
                                    f"All loading approaches failed for {strategy_name}"
                                )
                            ...
                            st.success(
                                f"✅ {selected_model_name} loaded successfully using {strategy_name}!"
                            )
                            break
                    ...
                    if "CUDA" in str(e):
                        ...
                        st.code(
                            "# Set environment variable to get better error info:\nset CUDA_LAUNCH_BLOCKING=1"
                        )
            ...
            try:
                ...
                if seed is not None:
                    ...
                    if (
                        ...
                        and hasattr(pipeline, "device")
                        and "cuda" in str(pipeline.device)
                    ):
                    ...
                    try:
                        ...
                        if hasattr(frames, "shape") and len(frames.shape) == 4:
                            ...
                            try:
                                ...
                                raise Exception(
                                    f"Cannot convert frames to list. Type: {type(frames)}"
                                )
                    ...
                    for i, frame in enumerate(frames):
                        try:
                            ...
                            if len(frame_array.shape) == 4:  # Batch dimension
                                ...
                                st.warning(
                                    f"⚠️ Frame {i} has invalid shape {frame_array.shape}, skipping..."
                                )
                                continue
                            ...
                            if len(frame_array.shape) != 3:
                                st.warning(
                                    f"⚠️ Frame {i} has unexpected dimensions {frame_array.shape}, skipping..."
                                )
                                continue
                            ...
                            if frame_bgr.dtype != np.uint8:
                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        max_val <= 255.0 and min_val >= 0.0
                                    ):  # Already in [0, 255] range
                                        frame_bgr = np.clip(frame_bgr, 0, 255).astype(np.uint8)
                                    else:  # Need to normalize
                                        frame_bgr = (
                                            (frame_bgr - min_val) / (max_val - min_val) * 255
                                        ).astype(np.uint8)
                                except Exception as conv_error:
                                    st.warning(
                                        f"⚠️ Frame {i} conversion error: {conv_error}, using fallback..."
                                    )
                                    frame_bgr = np.clip(frame_bgr, 0, 255).astype(np.uint8)
                            ...
                            if frame_bgr.shape[:2] != (height, width):
                                st.warning(
                                    f"⚠️ Frame {i} has wrong dimensions: {frame_bgr.shape[:2]}, expected: ({height}, {width})"
                                )
                                continue

                            if len(frame_bgr.shape) != 3 or frame_bgr.shape[2] != 3:
                                st.warning(
                                    f"⚠️ Frame {i} has wrong channel count: {frame_bgr.shape}"
                                )
                                continue
...
def rag_knowledge_base_page():
    ...
    if not RAG_HANDLER_AVAILABLE:
        st.error(
            "❌ RAG Handler not available. Please ensure rag_handler.py is in the project directory."
        )
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🔍 Query", "➕ Add Knowledge", "📂 Manage Documents", "📖 Help"]
    )
        ...
        if add_method == "✍️ Manual Entry":
            ...
            with col2:
                knowledge_category = st.text_input(
                    "Category (optional):", placeholder="e.g., features, technical, general"
                )
            ...
            if st.button("➕ Add to Knowledge Base", type="primary"):
                if knowledge_content.strip():
                    ...
                    doc_id = rag_handler.add_knowledge(
                        knowledge_content, source=knowledge_source, metadata=metadata
                    )
    ...
    with tab3:
        ...
        if rag_handler.db.data["documents"]:
            ...
            filter_text = st.text_input(
                "Filter documents:", placeholder="Search by content or ID...", key="filter_docs"
            )
            ...
            for idx, doc in enumerate(rag_handler.db.data["documents"]):
                ...
                if (
                    filter_text
                    and filter_text.lower() not in doc_id.lower()
                    and filter_text.lower() not in content.lower()
                ):
                    continue
                ...
                with st.expander(f"📄 {doc_id[:50]}..." if len(doc_id) > 50 else f"📄 {doc_id}"):
                    ...
                    st.text_area(
                        "", content_preview, height=150, key=f"preview_{idx}", disabled=True
                    )
            ...
            st.info(
                "📭 No documents in the knowledge base yet. Add some in the 'Add Knowledge' tab!"
            )
    ...
    with tab4:
        ...
        **RAG (Retrieval-Augmented Generation)** is a technique that enhances AI responses by:
        ...
        st.markdown("---")
        st.info(
            "💡 **Pro Tip**: Start by adding documentation, FAQs, or important information about your project to create a useful knowledge base!"
        )
...
def about_page():
        ...
        try:
            ...
            with col_pref1:
                ...
                if isinstance(last_used_model, dict):
                    last_used_model = last_used_model.get("value") or last_used_model.get(
                        "optionValue"
                    )
                if last_used_model:
    ...
    with col1:
        ...
        deps = {"Streamlit": st, "Torch": None, "Whisper": None, "Ollama": None}
