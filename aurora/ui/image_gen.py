...
try:
    from hardware_optimizer import get_hardware_info, get_hardware_optimizer, get_image_settings
...
def main():
        ...
        **Available Models:**
        ...
        if HARDWARE_OPTIMIZER_AVAILABLE and hardware_info.get("gpu", {}).get("cuda_available"):
            ...
            if vram_gb >= 16:
                ...
                st.info(
                    f"💡 Recommended: {max_images} images at {max_resolution[0]}x{max_resolution[1]}"
                )
            elif vram_gb >= 12:
                st.success("✅ Great GPU - Good resolution, multiple images")
                st.info(
                    f"💡 Recommended: {default_num_images} images at {default_resolution[0]}x{default_resolution[1]}"
                )
            elif vram_gb >= 8:
    ...
    with col1:
        ...
        default_value = (
            preferred_model
            if preferred_model and preferred_model in available_models.keys()
            else default_model
        )

        st.markdown("**🤖 Choose AI Model:**")
        model_selection = markdown_select(
            options=model_options, default_value=default_value, key="image_model_selection"
        )
        ...
        if show_custom:
                    ...
                    if custom_model_name and custom_model_id:
                        if IMAGE_MODEL_MANAGER_AVAILABLE:
                            with st.spinner(f"Adding {custom_model_name}..."):
                                success = image_model_manager.add_custom_model(
            ...
            selected_model_name = (
                list(available_models.keys())[0] if available_models else default_model
            )
                    ...
                    if not st.session_state[delete_key]:
                        ...
                        with col1_del:
                            if st.button(
                                "🗑️ Yes, Delete",
                                type="primary",
                                key=f"btn_confirm_img_{option_value}",
                            ):
                        ...
                        with st.spinner(f"Deleting {option_value}..."):
                            ...
                            keys_to_remove = [
                                ...
                                if key.startswith("image_model_selection")
                            ]
        ...
        if _preferences_manager and selected_model_name != current_last_used:
            _preferences_manager.set_preference(
                "image_generation", "last_used_model", selected_model_name
            )
        ...
        with style_col1:
            if st.button("📸 Photorealistic"):
                if prompt != "":
                    st.session_state["prompt"] = (
                        f"{prompt}, photorealistic, highly detailed, 8k resolution"
                    )
                    st.rerun()
        with style_col2:
    ...
    with col2:
        ...
        if _preferences_manager and num_images != image_prefs.get("preferred_num_images"):
            _preferences_manager.set_preference(
                "image_generation", "preferred_num_images", num_images
            )
            _preferences_manager.save_preferences()
        ...
        preferred_resolution = image_prefs.get(
        ...
        if _preferences_manager and resolution_key != image_prefs.get("preferred_resolution"):
            _preferences_manager.set_preference(
                "image_generation", "preferred_resolution", resolution_key
            )
            _preferences_manager.save_preferences()
    ...
    with st.expander("🔧 Advanced Settings (Hardware Optimized)"):
        ...
        with col_adv1:
            ...
            if _preferences_manager and guidance != image_prefs.get("preferred_guidance"):
                _preferences_manager.set_preference(
                    "image_generation", "preferred_guidance", guidance
                )
                _preferences_manager.save_preferences()
        ...
        with col_adv2:
            ...
            if use_custom:
                custom_model = st.text_input(
                    "Model ID:", placeholder="e.g., runwayml/stable-diffusion-v1-5"
                )
                if custom_model:
    ...
    if st.session_state.current_model_id != selected_model_id:
        with st.spinner(f"🔄 Loading {selected_model_name}..."):
            ...
            st.session_state.current_model_id = (
                selected_model_id if st.session_state.pipeline else None
            )
    ...
    if st.button("🚀 Generate Images", type="primary", use_container_width=True):
        ...
        if images:
            st.success(
                f"✅ Successfully generated {len(images)} image{'s' if len(images) > 1 else ''}!"
            )
            ...
            with st.expander("📊 Generation Details"):
                ...
                if IMAGE_MODEL_MANAGER_AVAILABLE:
                    generation_info["model_installed"] = image_model_manager.is_model_installed(
                        selected_model_name
                    )
                    model_size = image_model_manager.get_model_size(selected_model_name)
                    if model_size:
