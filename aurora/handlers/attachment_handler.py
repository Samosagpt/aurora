...
try:
    ...
    PDF_SUPPORT = False
    logging.warning(
        "PyPDF2 not available. PDF text extraction disabled. Install with: pip install PyPDF2"
    )
...
try:
    ...
    logging.warning(
        "pdf2image not available. PDF to image conversion disabled. Install with: pip install pdf2image"
    )
...
class AttachmentHandler:
    ...
    def __init__(self, cache_dir: Optional[Path] = None):
        ...
        """
        self.cache_dir = Path(cache_dir) if cache_dir else Path("logs/attachments")
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        # Track whether PDF->image conversion should be attempted (may require poppler)
        self._pdf_image_support = PDF_IMAGE_SUPPORT
    ...
    def process_image(self, image_path: Union[str, Path], resize: bool = True) -> Optional[Dict]:
        ...
        try:
            ...
            with Image.open(image_path) as img:
                ...
                # Determine resampling filter in a backward-compatible way
                try:
                    resample_filter = Image.Resampling.LANCZOS
                except AttributeError:
                    # Older Pillow releases expose LANCZOS at module level
                ...
                if resize and (
                    ...
                    ratio = min(
                        self.MAX_IMAGE_DIMENSION / img.width, self.MAX_IMAGE_DIMENSION / img.height
                    )
    ...
    def process_pdf(
        ...
        try:
            ...
            # Convert PDF pages to images (requires poppler/pdf2image)
            if extract_images and self._pdf_image_support:
                try:
                    from pdf2image import convert_from_path

                    images = convert_from_path(
                        str(pdf_path), dpi=150, first_page=1, last_page=min(max_pages, 5)
                    )
                    ...
                    for i, img in enumerate(images):
                        ...
                        result["images"].append(
                            {"page_num": i + 1, "data": img_base64, "size": img.size}
                        )

                    logger.info(f"Converted {len(images)} pages to images from {pdf_path.name}")
                except Exception as e:
                    # If conversion fails (commonly due to missing poppler), disable image conversion
                    logger.error(f"Error converting PDF to images: {e}")
                    result["images_unavailable_reason"] = str(e)
                    # Avoid future attempts in this process
                    self._pdf_image_support = False
            ...
            for attachment in attachments:
                if attachment["type"] == "image":
                    ...
                    if attachment.get("images"):
                        ...
                        # Note if images were unavailable (poppler missing or conversion failed)
                        reason = attachment.get("images_unavailable_reason")
                        if reason:
                            context_parts.append(f"[PDF images not available: {reason}]")

            # Combine context
            full_prompt = "\n\n".join(context_parts)

            result = {"prompt": full_prompt, "has_images": len(images) > 0}
