...
from aurora.utils.PreTrainedResponses import get_pretrained_response
from aurora.utils.ws import web_services
...
class PromptHandler:
    ...
    def __init__(self):
        self.intent_patterns = {
            ...
            "smart_search": [r"^smart search\s+(.+)", r"^deep search\s+(.+)", r"^research\s+(.+)"],
            "news": [
                r"^news$",
                r"^latest news$",
                r"^get news$",
    ...
    def _handle_weather_request(self, city: Optional[str]) -> str:
        ...
        if not city:
            return (
                "Please specify a city for the weather forecast. For example: 'weather in New York'"
            )
