...
class RAGDatabase:
    ...
    def __init__(self, db_path: str = "rag_db.json"):
        ...
        self.data = {
            "metadata": {"created": None, "updated": None, "version": "1.0", "total_documents": 0},
            "documents": [],
        }
        self.load_database()
    ...
    def load_database(self) -> bool:
        ...
        try:
            if self.db_path.exists():
                ...
                logger.info(
                    f"✅ Loaded RAG database: {self.db_path} ({self.data['metadata']['total_documents']} documents)"
                )
                return True
            else:
    ...
    def add_document(
        self, content: str, doc_id: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def add_documents_batch(self, documents: List[Dict[str, Any]]) -> List[str]:
        ...
        for doc in documents:
            doc_id = self.add_document(
                content=doc.get("content", ""), doc_id=doc.get("id"), metadata=doc.get("metadata")
            )
            if doc_id:
...
class RAGHandler:
    ...
    def add_knowledge(
        self, content: str, source: str = "manual", metadata: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def query(
        self, question: str, context_window: int = 3, return_sources: bool = True
    ) -> Dict[str, Any]:
...
def add_knowledge(
    content: str, source: str = "manual", metadata: Optional[Dict[str, Any]] = None
) -> str:
