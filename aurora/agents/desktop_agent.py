...
class DesktopAgent:
    ...
    def get_available_tools(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "parameters": {"app_name": "Application name or window title"},
            },
            {"name": "list_windows", "description": "List all open windows", "parameters": {}},
            {
                "name": "switch_window",
                "description": "Switch to a specific window",
                "parameters": {"window_title": "Window title or partial title"},
            },
            {
    ...
    def mouse_click(
        self, x: int = None, y: int = None, button: str = "left", clicks: int = 1
    ) -> Dict[str, Any]:
    ...
    def take_screenshot(
        self, region: Tuple[int, int, int, int] = None, save_path: str = None
    ) -> Dict[str, Any]:
        """Take screenshot"""
        try:
    ...
    def ocr_screen(
        self, region: Tuple[int, int, int, int] = None, search_text: str = None
    ) -> Dict[str, Any]:
    ...
    def click_text(self, text: str, button: str = "left", confidence: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            click_result = self.mouse_click(x=coords["x"], y=coords["y"], button=button)
    ...
    def recognize_gui_elements(self, region: Tuple[int, int, int, int] = None) -> Dict[str, Any]:
        ...
        try:
                ...
                if 100 < area < (img.shape[0] * img.shape[1] * 0.5):
                    ...
                    element_img = (
                        ...
                        if not region
                            ...
                            y - region[1] : y - region[1] + h, x - region[0] : x - region[0] + w
                        ]
                    )
            ...
            self._log_action(
                "recognize_gui_elements", {"region": region, "found": len(gui_elements)}
            )
    ...
    def open_application(self, app_name: str, args: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(exe_path, str) and exe_path.startswith("start "):
                ...
                self._log_action(
                    "open_application", {"app_name": app_name, "args": args, "pid": process.pid}
                )
                ...
                for hwnd, title in windows:
                    ...
                    if any(keyword in title_lower for keyword in browser_keywords):
                        try:
                            ...
                            logger.info(
                                f"Successfully brought browser window to foreground: {title[:50]}"
                            )
                            break
                        except Exception as e:
            ...
            def enum_windows_callback(hwnd, results):
                if win32gui.IsWindowVisible(hwnd):
                    ...
                    if title:
                        ...
                        results.append(
                            {"hwnd": hwnd, "title": title, "pid": pid, "process": process_name}
                        )
        ...
        try:
            ...
            if hwnd:
                ...
                if is_minimized:
                    ...
                    logger.info(
                        f"Window is not minimized, just bringing to front: {window_title} (hwnd={hwnd})"
                    )
    ...
    def write_file(self, file_path: str, content: str, mode: str = "w") -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_action(
                "write_file", {"file_path": file_path, "size": len(content), "mode": mode}
            )
    ...
    def list_directory(self, directory_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_action(
                "list_directory", {"directory_path": directory_path, "count": len(items)}
            )
    ...
    def search_files(self, search_path: str, pattern: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_action(
                "search_files",
                {"search_path": search_path, "pattern": pattern, "count": len(files)},
            )
    ...
    def _log_action(self, action_name: str, params: Dict[str, Any]) -> None:
        ...
        action = {"action": action_name, "params": params, "timestamp": time.time()}
