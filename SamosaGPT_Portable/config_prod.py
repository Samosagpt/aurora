...
class ProductionConfig:
    ...
    def get_safe_config(cls) -> Dict[str, Any]:
        ...
        for attr in dir(cls):
            if not attr.startswith('_') and not callable(getattr(cls, attr)):
                ...
                if any(sensitive in attr.lower() for sensitive in ['key', 'token', 'secret', 'password']):
    ...
    def validate_api_keys(cls) -> Dict[str, bool]:
        """Validate API keys"""
        return {
            "openweather": bool(cls.OPENWEATHER_API_KEY and cls.OPENWEATHER_API_KEY != "YOUR_API_KEY"),
