...
class HardwareOptimizer:
    ...
    def print_hardware_summary(self) -> None:
        ...
        print(f"🖥️  System: {system.get('platform', 'Unknown')} {system.get('platform_version', '')}")
        ...
        if 'frequency_max' in cpu:
            print(f"⚡ Frequency: {cpu.get('frequency_current', 0):.0f} MHz (max: {cpu.get('frequency_max', 0):.0f} MHz)")
        ...
        if gpu.get('cuda_available'):
            ...
            for gpu_device in gpu.get('gpus', []):
                print(f"   📱 {gpu_device.get('name', 'Unknown')}: {gpu_device.get('vram_gb', 0):.1f} GB VRAM")
            print(f"🔥 Total VRAM: {gpu.get('total_vram_gb', 0):.1f} GB")
            print(f"🔧 CUDA: {gpu.get('cuda_version', 'Unknown')} | PyTorch: {gpu.get('torch_version', 'Unknown')}")
        else:
        ...
        if self.optimized_settings:
            ...
            print(f"💬 Chat: {len(chat.get('recommended_models', []))} models, {chat.get('context_length', 0)} context")
            print(f"🎨 Images: {image.get('default_num_images', 0)} default, {image.get('max_images', 0)} max, {image.get('recommended_resolution', [0,0])[0]}x{image.get('recommended_resolution', [0,0])[1]} resolution")
            print(f"🎬 Video: {video.get('recommended_frames', 0)} frames, {video.get('recommended_fps', 0)} fps, {video.get('recommended_resolution', [0,0])[0]}x{video.get('recommended_resolution', [0,0])[1]} resolution")
