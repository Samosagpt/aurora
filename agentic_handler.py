...
"""
    ...
    def process_user_request(self, user_input: str, model: str = None) -> str:
        ...
        try:
            ...
            print(f"🔍 DEBUG [agentic_handler]: System prompt generated ({len(system_prompt)} chars)")
...
def is_desktop_control_request(user_input: str) -> bool:
    ...
    control_keywords = [
        'open', 'close', 'click', 'type', 'press', 'screenshot',
        'window', 'application', 'file', 'folder', 'directory',
        'command', 'run', 'execute', 'mouse', 'keyboard', 'move',
        'search', 'find', 'list', 'show', 'read', 'write', 'create'
    ]
...
def handle_agentic_request(user_input: str, model: str = None) -> str:
    ...
    print(f"🔍 DEBUG [agentic_handler]: handle_agentic_request called with input: '{user_input}', model: '{model}'")
