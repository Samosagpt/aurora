...
class SamosaGPTInstaller:
    def __init__(self):
        ...
        self.start_menu_path = os.path.join(
    ...
    def setup_gui(self):
        ...
        title_label = tk.Label(
            title_frame, text="🤖 SamosaGPT Installer", font=("Arial", 18, "bold")
        )
        title_label.pack()
        ...
        options_frame = tk.LabelFrame(
            self.root, text="Installation Options", font=("Arial", 10, "bold")
        )
        ...
        tk.Checkbutton(
            options_frame, text="Create Desktop Shortcut", variable=self.create_desktop_shortcut
        ).pack(anchor=tk.W, padx=10, pady=2)
        tk.Checkbutton(
            options_frame, text="Add to Start Menu", variable=self.create_start_menu
        ).pack(anchor=tk.W, padx=10, pady=2)
        tk.Checkbutton(
            options_frame,
            text="Auto-start browser after installation",
            variable=self.auto_start_browser,
        ).pack(anchor=tk.W, padx=10, pady=2)
        ...
        self.status_label = tk.Label(
            progress_frame, textvariable=self.status_var, font=("Arial", 9)
        )
    ...
    def run_setup(self):
        ...
        try:
            ...
            result = subprocess.run(
                [setup_bat_path], cwd=self.appdata_path, capture_output=True, text=True, timeout=600
            )
    ...
    def create_shortcuts(self):
        ...
        try:
            ...
            if self.create_start_menu.get():
                ...
                start_menu_shortcut.Targetpath = os.path.join(
                    self.program_files_path, "SamosaGPT.bat"
                )
...
"""
    ...
    def show_completion_dialog(self):
        ...
        if result and self.auto_start_browser.get():
            try:
                subprocess.Popen(
                    [os.path.join(self.program_files_path, "SamosaGPT.bat")], cwd=self.appdata_path
                )
            except Exception as e:
